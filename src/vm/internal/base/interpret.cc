...
thread_local int call_origin = 0;
thread_local error_context_t *current_error_context = nullptr;
...
thread_local program_t *current_prog;
thread_local short int caller_type;
thread_local int tracedepth;
thread_local int num_varargs;
 ...
 */
thread_local char *pc;     /* Program pointer. */
thread_local svalue_t *fp; /* Pointer to first argument. */

thread_local svalue_t *sp;

thread_local int function_index_offset; /* Needed for inheritance */
thread_local int variable_index_offset; /* Needed for inheritance */
thread_local int st_num_arg;
...
thread_local ref_t *global_ref_list = nullptr;
 ...
 * arguments.
 */
static thread_local int previous_instruction[60];
static thread_local int stack_size[60];
static thread_local char *previous_pc[60];
static thread_local int last;
        ...
        if (!(funflags & FUNC_TRUE_VARARGS) && funcp->min_arg != funcp->num_arg) {
          ...
          if (pushed_args != funcp->num_arg) {
            ...
            for (int i = pushed_args; i < funcp->num_arg; i++) {
              ...
              if (sv_funcp.u.refed->ref.load() != 1) {
                fatal("F_CALL_FUNCTION_BYADDRESS: default args closure ref count %u != 1.\n", sv_funcp.u.refed->ref.load());
              }
...
void call_direct(object_t *ob, int offset, int origin, int num_arg) {
  // Cross-thread call detection: if a heartbeat worker thread calls a
  // function on an object owned by a different thread, bounce the entire
  // heartbeat to the main thread and abort the current path.
  if (g_heartbeat_thread_pool && g_current_heartbeat_thread) {
    ...
    if (target_thread != g_current_heartbeat_thread) {
      bounce_heartbeat_to_main_thread(ob);
      throw "heartbeat_bounce";
    }
  }
...
void reset_machine(int first) {
  // Bind stack pointers to the current thread's thread_local arrays.
  // end_of_stack and control_stack are shared globals; each thread
  // must call reset_machine() before using the VM to point them at
  // its own stacks.

...
#include "vm/internal/base/object.h"  // for sizeof(object_t) in thread_for_object hash
...
void HeartbeatThread::start() {
  ...
  // Reset per-instance debug counters.
  pending_call_count_ = 0;
  heartbeat_call_count_ = 0;
  modify_call_count_ = 0;
  ready_ = false;
  running_ = true;

  // Use pthread_create with a 2MB stack to avoid stack overflow
  // on the deep C++ call chain during heartbeat LPC execution.
  {
    pthread_attr_t attr;
    pthread_attr_init(&attr);
    pthread_attr_setstacksize(&attr, 2 * 1024 * 1024);  // 2MB
    auto fn = [](void *arg) -> void * {
      static_cast<HeartbeatThread *>(arg)->event_loop();
      return nullptr;
    };
    pthread_create(&thread_, &attr, fn, this);
    pthread_attr_destroy(&attr);
  }
  thread_id_ = thread_;

  // Spin until the thread signals readiness — ensures post() won't
  // write to the wakeup socket before the event loop is watching it.
  {
    int waits = 0;
    while (!ready_.load(std::memory_order_acquire)) {
      if (++waits > 1000) {  // ~1 second at 1ms spin
        fprintf(stderr, "HB-T%d start: WARNING thread not ready after 1s\n", id_);
        fflush(stderr);
        break;
      }
...
void HeartbeatThread::stop() {
  ...
  if (thread_) { pthread_join(thread_, nullptr); thread_ = {}; }
...
void HeartbeatThread::post(std::function<void()> task) {
  // Wait for the thread to be ready (event_loop watching the wakeup fd).
  // Without this, writes to the socket may land before kqueue is registered
  // and never trigger wakeup_cb.
  {
    int waits = 0;
    while (!ready_.load(std::memory_order_acquire)) {
      if (++waits > 1000) {
        fprintf(stderr, "HB-T%d post: WARNING thread not ready after 1s, dropping task\n", id_);
        fflush(stderr);
        return;  // drop — nowhere to send it
      }
      std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
  }
  ...
  if (has_fd) {
    char c = 1;
    // Retry on EINTR and EAGAIN (socket buffer full).
    while (write(wakeup_fds_[1], &c, 1) < 0) {
      if (errno == EINTR) continue;
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        // Socket buffer full — data from previous writes is still pending.
        // The thread will drain it and process all queued tasks, so we
        // don't actually need this byte to arrive. Safe to drop.
        break;
      }
      // Real error — log and drop.
      fprintf(stderr, "HB-T%d post: write error errno=%d (%s)\n",
...
void HeartbeatThread::event_loop() {
  ...
  fprintf(stderr, "HB-T%d: event_loop starting\n", id_);
  fflush(stderr);
...
void HeartbeatThread::process_pending_tasks() {
  ...
  // Atomically extract pending tasks.
  ...
  pending_call_count_++;
  if (pending_call_count_ <= 5 || pending_call_count_ % 100 == 0) {
    fprintf(stderr, "HB-T%d process_pending #%d: %zu tasks\n",
            id_, pending_call_count_, tasks.size());
    fflush(stderr);
  }
...
void HeartbeatThread::add_heartbeat(object_t *ob, int interval) {
  heartbeats_next_.push_back({ob, (short)interval, (short)interval});
}

void HeartbeatThread::remove_heartbeat(object_t *ob) {
  for (auto &hb : heartbeats_) if (hb.ob == ob) hb.ob = nullptr;
  for (auto &hb : heartbeats_next_) if (hb.ob == ob) hb.ob = nullptr;
}

void HeartbeatThread::modify_heartbeat(object_t *ob, int interval) {
  for (auto &hb : heartbeats_)
    if (hb.ob == ob) {
      hb.time_to_heart_beat = hb.heart_beat_ticks = (short)interval;
      return;
    }
  ...
  modify_call_count_++;
  if (modify_call_count_ <= 30) {
    fprintf(stderr, "HB-T%d modify #%d: added ob=%p interval=%d (next_queue=%zu)\n",
            id_, modify_call_count_, (void *)ob, interval, heartbeats_next_.size());
    fflush(stderr);
  }
}
...
void HeartbeatThread::process_heartbeats() {
  ...
  if (heartbeat_call_count_ <= 5 || heartbeat_call_count_ % 100 == 0)
    fprintf(stderr, "HB-T%d: process_heartbeats #%d gate=%d queue=%zu\n",
            id_, heartbeat_call_count_, g_heartbeat_gate.load(), heartbeats_.size());
  fflush(stderr);
  ...
  while (!heartbeats_.empty()) {
    obj_count++;
    if (obj_count % 500 == 0) {
      fprintf(stderr, "HB-T%d: heartbeat obj #%d\n", id_, obj_count);
      fflush(stderr);
    }
    ...
    save_context(&econ);
    try {
      if (obj_count <= 5 || obj_count % 500 == 0) {
        fprintf(stderr, "HB-T%d: calling heart_beat #%d ob=%s\n",
                id_, obj_count, ob->obname);
        fflush(stderr);
      }
  ...
  fprintf(stderr, "HB-T%d: process_heartbeats COMPLETE queue=%zu\n",
          id_, heartbeats_next_.size());
  fflush(stderr);
}

// Thread-safe call(fn) / call(delay, fn) — gametick-independent alternative to call_out.
void call(int delay_sec, std::function<void()> fn) {
  struct event_base *base = nullptr;
  if (g_current_heartbeat_thread) {
    base = g_current_heartbeat_thread->base();
  } else {
    base = g_event_base;
  }
  if (!base) { fn(); return; }  // fallback: execute immediately
  auto *f = new std::function<void()>(std::move(fn));
  struct timeval tv = {delay_sec, 0};
...
// Pool
HeartbeatThreadPool::HeartbeatThreadPool(size_t n) {
  for (size_t i = 0; i < n; i++)
    threads_.push_back(std::make_unique<HeartbeatThread>((int)i));
}
...
HeartbeatThread *HeartbeatThreadPool::thread_for_object(object_t *ob) {
  // Divide by sizeof(object_t) to get the object index, then hash.
  // Raw pointer hashing fails because all ptrs are 8-byte aligned AND
  // sizeof(object_t) is divisible by 8, so (ptr * K) % N === 0 always.
  auto idx = static_cast<size_t>(reinterpret_cast<uintptr_t>(ob) / sizeof(object_t));
  return threads_[idx % threads_.size()].get();
}
...
void bounce_heartbeat_to_main_thread(object_t *ob) {
  ...
  event_base_once(g_event_base, -1, EV_TIMEOUT,
                  [](evutil_socket_t, short, void *arg) {
                    auto *o = (object_t *)arg;
                    if (!(o->flags & O_DESTRUCTED) && o->prog->heart_beat != 0) {
                      error_context_t e;
                      save_context(&e);
                      try {
                        set_eval(max_eval_cost);
                        call_direct(o, o->prog->heart_beat - 1, 0, 0);
                        pop_stack();
                      } catch (const char *) {
                        restore_context(&e);
                      }
